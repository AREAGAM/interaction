...
function setMode(next: Mode) {
  ...
  if (next !== "boot") {
    bootSequence.reset();
    $(".file-title").firstChild!.textContent = "PROJECT FILE: ";
...
function renderDetail() {
  ...
  <dl class="metadata"><div><dt>DISCIPLINE / 设计领域</dt><dd>${escapeHtml(r.department)}</dd></div><div><dt>YEAR / 项目时间</dt><dd>${escapeHtml(r.date)}</dd></div><div><dt>ROLE / 个人职责</dt><dd>${escapeHtml(r.lead)}</dd></div><div><dt>STATUS / 状态</dt><dd><i></i>${r.clearance === "RESTRICTED" ? "目录访问" : "概念占位 · 可替换"}</dd></div></dl>
...
function setTab(tab: string, sound = true) {
  ...
  $("#tab-panel").innerHTML =
    tab === "overview"
      ? overview()
      : tab === "notes"
        ? `<div class="panel-label">DESIGN PROCESS / 设计过程</div><ol class="research-notes">${r.findings.map((f, i) => `<li><span>${String(i + 1).padStart(2, "0")}</span>${escapeHtml(f)}</li>`).join("")}</ol>`
        : `<div class="panel-label">ACCESS LOG / 本次访问</div>${accessLog
            .filter((entry) => entry.id === r.id)
            .slice(0, 4)
            .map(
              (entry) =>
                `<div class="log-row"><span>${entry.time}</span><span>VISITOR / 访客</span><b>PROJECT OPENED</b></div>`,
            )
            .join(
              "",
            )}<p class="log-note">仅记录本次浏览。当前作品和图像为占位示例，后续可替换为真实项目。</p>`;
...
function renderResults() {
  ...
  $("#search-results").innerHTML = results.length
    ? results
        ...
        .join("")
    : `<div class="empty-results"><span>∅</span><strong>${modal === "saved" && !searchQuery ? "尚无收藏档案" : "没有匹配的档案"}</strong><p>${modal === "saved" && !searchQuery ? "读取档案时，选择 SAVE ARCHIVE 将其保存在此处。" : "尝试其他名称、档案编号，或切换设计领域分类。"}</p><button data-action="reset-search">${modal === "saved" ? "查看全部档案 →" : "重置检索 →"}</button></div>`;
...
function settingsMarkup() {
  return `<h2>SYSTEM SETTINGS<small>终端偏好设置</small></h2><p class="settings-intro">VISITOR / 访客 <span>·</span> SEE / SHOW / DESIGN STUDIO</p><div class="settings-list">${audioSettingsMarkup(prefs)}<label><div><strong>REDUCED MOTION</strong><span>减少镜头移动和过渡动效</span></div><input type="checkbox" data-pref="reduced" ${prefs.reduced ? "checked" : ""}/><i class="toggle"></i></label></div>${qualityMarkup(prefs.rendering)}<div class="settings-shortcuts"><span>KEYBOARD CONTROLS</span><p><kbd>←</kbd><kbd>→</kbd> 切列 <kbd>↑</kbd><kbd>↓</kbd> 选档 <kbd>ENTER</kbd> 读取 <kbd>/</kbd> 检索 <kbd>ESC</kbd> 返回</p></div><div class="settings-bottom"><button data-action="fullscreen">FULLSCREEN <span>↗</span></button><button data-action="restart">REINITIALIZE SYSTEM <span>↻</span></button></div><div class="modal-bottom"><span>DESIGN ARCHIVE / 1.0 · 使用 MiSans 字体（小米） <a href="/fonts/MiSans-license.pdf" target="_blank" rel="noopener">字体许可</a></span><span>POWERED BY SEE / SHOW</span></div>`;
}
...
document.addEventListener("click", (e) => {
  ...
  if (action === "model-viewer" && mode === "detail") {
    ...
    viewer.open(
      ...
      () => scene.createAssemblyModel(records[selected]),
      prefs.reduced,
    );
    audio.play("page-open");
  }
